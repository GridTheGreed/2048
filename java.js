...
function slideUp(){
    for(let c = 0; c < columns; c++) {
        ...
        // chech which tiles have changed in column
        let changedIndices = [];
        for(let r = 0; r < rows; r++){  
...
function restartGame(){
    ...
    score = 0;  // babalik lang natin si score sa zero
    setTwo()    // new tile   
}
...
document.addEventListener("touchmove", (e) =>{
    ...
    // disables the line scrolling
    e.preventDefault(); 
},{passive:false});

// actual na pagreceive ng swipe sa user

document.addEventListener("touchend", (e)=>{
    // check if the element triggered the event ha a classname tile
